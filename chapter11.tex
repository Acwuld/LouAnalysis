...
\begin{quiza}
\woe 利用解析函数的性质证明: \(\int_{0}^{+\infty}\frac{\ee^{-x}-\cos x}{x}\dd x=0\).
\begin{proof}
考虑积分\(\int_{L}\frac{\ee^{\ii z}}{z}\dd z\), 其中\(L\)是如下图所示的路径:
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale=0.8]
\draw[-latex,thick] (-1,0)--(6,0);
\draw[-latex,thick] (0,-1)--(0,6);
\draw[thick,color=red] (0.8,0) arc (0:45:0.8);
\draw[thick,->,color=red] (0,0.8) arc (90:45:0.8);
\draw[thick,->,color=red] (5,0) arc (0:45:5);
\draw[thick,color=red] (0,5) arc (90:45:5);
\draw[->,thick,color=red] (0.8,0)--(2.5,0);
\draw[thick,color=red] (2,0)--(5,0);
\draw[thick,color=red] (0,0.8)--(0,2.5);
\draw[<-,thick,color=red] (0,2.5)--(0,5);

\node at (-0.3,-0.3) {$O$};
\node at (6,-0.3) {$x$};
\node at (-0.3,6) {$y$};
\node at (3.5,4) {$\Gamma_R$};
\node at (0.8,0.7) {$\Gamma_r$};
\node at (0.8,-0.3) {$A$};
\node at (5,-0.3) {$B$};
\node at (-0.3,5) {$C$};
\node at (-0.3,0.8) {$D$};
...
\caption{}
\end{figure}
其中\(\Gamma_R\)表图中半径为\(R\)的弧. 由于\(f(z)=\frac{\ee^{\ii z}}{z}\)在\(\Gamma_R\)所围区域内解析, 从而\[\int_{\Gamma_R}\frac{\ee^{\ii z}}{z}\dd z=0.\]又\[\int_{L}f(z)\dd z=\int_{AB}f(z)\dd z+\int_{\Gamma_R}f(z)\dd z+\int_{CD}f(z)\dd z\dd z+\int_{\Gamma_r}f(z)\dd z,\]
注意到\[\begin{split}
&\int_{AB}f(z)\dd z+\int_{CD}f(z)\dd z =\int_{r}^{R}\frac{\ee^{\ii x}}{x}\dd x+\int_{R}^{r}\frac{\ee^{-y}}{y}\dd y\\=&\int_{r}^{R}\frac{-\ee^{-x}+\ee^{\ii x}}{x}\dd x=-\int_{r}^{R}\frac{\ee^{-x}-\cos x}{x}\dd x+\ii\int_{r}^{R}\frac{\sin x}{x}\dd x,
\end{split}\]

下面我们说明\(\int_{\Gamma_r}f(z)\dd z=-\frac{\pi\ii }{2}\), 由于\(\lim_{r\rightarrow 0}zf(z)=1\), 则\(r\)充分小时, 有\[\left|z f(z)-1\right|<\frac{2}{\pi}\varepsilon,\]又\[\left|\int_{\Gamma_r}f(z)\dd z-\ii\frac{\pi}{2}\right|=\left|\int_{\Gamma_r}\frac{zf(z)-1}{z}\dd z\right|<\varepsilon,\]从而结论得证. 又由Jordan引理可知\[\lim_{R\rightarrow+\infty}\frac{\ee^{\ii z}}{z}\dd z=0,\]结合上述结论可知\(\int_{0}^{+\infty}\frac{\ee^{-x}-\cos x}{x}\dd x=0\).

%\(g(z)=\frac{\ee^{-z-\ii z}-1}{z}\), 下面我们证明\[\lim_{R\rightarrow+\infty}\int_{\Gamma_R}g(z)\ee^{\ii z}\dd z =\lim_{R\rightarrow+\infty}\int_{\Gamma_R}f(z)\dd z=0.\]由于\(\lim_{R\rightarrow+\infty}g(z)=0\)
\end{proof}
