...
\begin{proof}
考虑积分\(\int_{\Gamma_R}\frac{\ee^{-z}-\ee^{\ii z}}{z}\dd z\), 其中\(\Gamma_R\)是如下图所示的路径:
\begin{figure}[H]
\centering
...
\node at (-0.3,5) {$C$};
\node at (-0.3,0.8) {$D$};
\node at (4.5,5) {$\Gamma_R$};
\end{tikzpicture}
\caption{}
\end{figure}
由于\(f(z)=\frac{\ee^{-z}-\ee^{\ii z}}{z}\)在\(\Gamma_R\)所围区域内解析, 从而\[\int_{\Gamma_R}\frac{\ee^{-z}-\ee^{\ii z}}{z}\dd z=0.\]又\[\int_{\Gamma_R}f(z)\dd z=\int_{AB}f(z)\dd z+\int_{\overset{\LARGE{\frown}}{BC}}f(z)\dd z+\int_{CD}f(z)\dd z\dd z+\int_{\overset{\LARGE{\frown}}{DA}}f(z)\dd z,\]
注意到\[\begin{split}
\int_{AB}f(z)\dd z+\int_{CD}f(z)\dd z &=\int_{r}^{R}\frac{\ee^{-x}-\ee^{\ii x}}{x}\dd x+\int_{R}^{r}\frac{\ee^{-\ii y}-\ee^{-y}}{y}\dd y\\&=2\int_{r}^{R}\frac{\ee^{-x}-\cos x}{x}\dd x,
\end{split}\]

下面我们说明\(\int_{\overset{\LARGE{\frown}}{BC}}f(z)\dd z=\int_{\overset{\LARGE{\frown}}{DA}}f(z)\dd z=0\),
\end{proof}
